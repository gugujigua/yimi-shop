  ...
  <el-card class="box-card">
    ...
    <el-form ref="editform" :model="editform"  label-width="80px">
      <el-form-item label="分类" prop="cat_id">
        <el-select v-model="editform.cat_id" placeholder="请选择分类">
            <el-option
            v-for="item in cates"
            :key="item.id"
            :label="item.catename"
      ...
      <el-form-item label="标题" prop="title">
        <el-input v-model="editform.title" placeholder="最多输入50个字符" maxlength="50"></el-input>
      </el-form-item>
      <el-form-item label="描述" prop="description">
        <el-input type="textarea" v-model="editform.description" maxlength="200" placeholder="最多输入200个字符"></el-input>
      </el-form-item>
...
export default {
  ...
  mounted() {
    ...
    this.$api.news.one(this.$route.params.id).then(res=>{
        if(res.data.code == 200){
  ...
  methods: {
    editSubmit() {
        var that = this;
        delete this.editform.catename;
      this.$api.news.edit(this.$route.params.id,this.editform).then(res => {
        if (res.data.code == 200) {
            this.$message({
                type: "success",
